...
for (let i = 0; i < jsonData.length; i++) {
    ...
    let marker = L.marker([team.lat, team.long], { icon: getCustomIcon(team) }).addTo(map);
...
for (let i = 0; i < jsonData.length; i++) {
    ...
    if (team.StanleyCupWins > 4) {
        let scMarker = L.marker([team.lat, team.long], { icon: getCustomIcon(team) }).addTo(map);
...
// Layer 3: % of Playoff Seasons
for (let i = 0; i < jsonData.length; i++) {
    let team = jsonData[i]
    if (team.playoffPercent > .5) {
        let playoffMarker = L.marker([team.lat, team.long], { icon: getCustomIcon(team) }).addTo(map);
...
for (let i = 0; i < jsonData.length; i++) {
    ...
    if (team.totalSeasons > 49) {
        let seasonsMarker = L.marker([team.lat, team.long], { icon: getCustomIcon(team) }).addTo(map);
